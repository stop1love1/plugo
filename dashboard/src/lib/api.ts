...
export const getSetupStatus = () => api.get<{ has_users: boolean }>("/auth/setup-status").then((r) => r.data);
...
export const createUser = (data: { username: string; password: string; role: string }) =>
  api.post<{ id: string; message: string }>("/users", data).then((r) => r.data);
export const updateUserRole = (id: string, role: string) =>
  api.put<{ message: string }>(`/users/${id}/role`, { role }).then((r) => r.data);
export const deleteUser = (id: string) => api.delete<{ message: string }>(`/users/${id}`).then((r) => r.data);
