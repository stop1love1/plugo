...
export default function GlobalSettings() {
  ...
  const handleSave = useCallback(() => {
    ...
    if (Object.keys(changes).length === 0) {
      toast(t("globalSettings.noChanges"));
      return;
    }
    ...
    <div className="flex gap-6 -m-4 lg:-m-8 min-h-screen">
      ...
      <div className="w-52 shrink-0 bg-white border-r border-gray-200 p-4 sticky top-0 h-screen overflow-y-auto">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3 px-2">
          {t("globalSettings.sidebarHeading")}
        </h2>
      ...
      <div className="flex-1 py-6 pr-6 space-y-8 overflow-y-auto">
        ...
        {hasChanges && (
          <div className="px-4 py-2 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-700 flex items-center gap-2">
            ...
            {t("globalSettings.unsaved")}{" "}
            <kbd className="px-1.5 py-0.5 bg-amber-100 rounded text-xs font-mono">Ctrl+S</kbd>{" "}
            {t("globalSettings.ctrlSToSave")}
          </div>
        )}
        ...
        >
          <SectionCard
            ...
            desc="Global system prompt template and fallback responses. Per-site custom prompts are appended after this."
          >
            <TextAreaField
            ...
            <InputField
              ...
              value={
                Array.isArray(get("agent", "no_tool_providers"))
                  ? (get("agent", "no_tool_providers") as string[]).join(", ")
                  : String(get("agent", "no_tool_providers", ""))
              }
              onChange={(v) =>
                update(
                  "agent",
        ...
        >
          <SectionCard
            icon={Cpu}
            title="Default LLM"
            desc="Default model for new sites. Each site can override in its own settings."
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField
                label="Provider"
                value={get("llm", "provider", "claude")}
                onChange={(v) => update("llm", "provider", v)}
                placeholder="claude"
                hint="claude, openai, gemini, ollama, lmstudio"
              />
              <InputField
                label="Model"
                value={get("llm", "model", "")}
          ...
          <div className="mt-4">
            <SectionCard
              ...
              desc="Settings for locally hosted LLM servers."
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InputField
                  label="Ollama Base URL"
                  value={get("ollama", "base_url", "http://localhost:11434")}
                  onChange={(v) => update("ollama", "base_url", v)}
                  placeholder="http://localhost:11434"
                />
                <InputField
                  label="Default Ollama Model"
                  value={get("ollama", "model", "llama3")}
        ...
        <div
          ref={(el) => {
            sectionRefs.current["embedding"] = el;
          }}
          className="scroll-mt-6"
        >
          <SectionCard
            icon={Zap}
            title="Embedding"
            desc="Vector embeddings for knowledge base search (RAG). Restart required after changes."
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField
                label="Provider"
                value={get("embedding", "provider", "openai")}
                onChange={(v) => update("embedding", "provider", v)}
                placeholder="openai"
                hint="openai, gemini, ollama"
              />
              <InputField
                label="Model"
                value={get("embedding", "model", "")}
                onChange={(v) => update("embedding", "model", v)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField
                label="Cache Size"
                type="number"
                value={get("embedding", "cache_size", 1000)}
                onChange={(v) => update("embedding", "cache_size", parseInt(v) || 1000)}
                hint="Number of embeddings cached in memory"
              />
              <InputField
                label="Cache TTL (seconds)"
                type="number"
                value={get("embedding", "cache_ttl", 3600)}
                onChange={(v) => update("embedding", "cache_ttl", parseInt(v) || 3600)}
              />
            </div>
          </SectionCard>
        </div>
        ...
        >
          <SectionCard
            icon={Database}
            title="Database"
            desc="Primary data store. Changing provider requires a server restart."
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SelectField
                label="Provider"
                value={get("database", "provider", "sqlite")}
                onChange={(v) => update("database", "provider", v)}
                options={[
              ...
              <InputField
                label="SQLite URL"
                value={get("database", "url", "")}
                onChange={(v) => update("database", "url", v)}
                disabled={get("database", "provider") !== "sqlite"}
                mono
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField
                label="MongoDB URL"
                value={get("database", "mongodb_url", "")}
                onChange={(v) => update("database", "mongodb_url", v)}
                disabled={get("database", "provider") !== "mongodb"}
                placeholder="mongodb://localhost:27017"
                mono
              />
              <InputField
                label="MongoDB Database"
                value={get("database", "mongodb_database", "")}
                onChange={(v) => update("database", "mongodb_database", v)}
                disabled={get("database", "provider") !== "mongodb"}
                placeholder="plugo"
              />
            </div>
        ...
        >
          <SectionCard
            icon={Brain}
            title="RAG Pipeline"
            desc="Controls how knowledge base search retrieves and ranks content."
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputField
                label="Min Score"
                type="number"
                value={get("rag", "min_score", 0.3)}
                onChange={(v) => update("rag", "min_score", parseFloat(v) || 0.3)}
                hint="Minimum relevance (0-1)"
              />
              <InputField
                label="Max Chunks"
                type="number"
                value={get("rag", "max_chunks", 7)}
                onChange={(v) => update("rag", "max_chunks", parseInt(v) || 7)}
                hint="Max context chunks"
              />
              <InputField
                label="ChromaDB Path"
                value={get("vector_store", "chroma_path", "./data/chroma")}
                onChange={(v) => update("vector_store", "chroma_path", v)}
                mono
              />
            </div>
          </SectionCard>
        ...
        >
          <SectionCard
            ...
            desc="Backend port, CORS, and widget delivery. Restart required."
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InputField
                label="Backend Port"
                type="number"
                value={get("server", "backend_port", 8000)}
                onChange={(v) => update("server", "backend_port", parseInt(v) || 8000)}
              />
              <InputField
                label="Widget CDN URL"
                value={get("server", "widget_cdn_url", "")}
                onChange={(v) => update("server", "widget_cdn_url", v)}
                hint="Public URL to widget.js"
                mono
              />
            </div>
            <div>
              ...
              <textarea
                value={
                  Array.isArray(get("server", "cors_origins"))
                    ? (get("server", "cors_origins") as string[]).join("\n")
        ...
        >
          <SectionCard
            ...
            desc="Default crawling behavior for all sites."
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <SelectField
                label="Verify SSL"
                value={get("crawl", "verify_ssl", true) ? "true" : "false"}
                onChange={(v) => update("crawl", "verify_ssl", v === "true")}
                options={[
              ...
              <InputField
                ...
                type="number"
                value={get("crawl", "request_delay", 1.0)}
                onChange={(v) => update("crawl", "request_delay", parseFloat(v) || 1.0)}
              />
              <InputField
                label="Request Timeout (sec)"
                type="number"
                value={get("crawl", "request_timeout", 30)}
                onChange={(v) => update("crawl", "request_timeout", parseInt(v) || 30)}
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputField
                label="Max Concurrent Fetches"
                type="number"
                value={get("crawl", "max_concurrent_fetches", 5)}
                onChange={(v) => update("crawl", "max_concurrent_fetches", parseInt(v) || 5)}
              />
              <InputField
                label="Max Auto Crawls"
                type="number"
                value={get("crawl", "max_concurrent_auto_crawls", 3)}
                onChange={(v) => update("crawl", "max_concurrent_auto_crawls", parseInt(v) || 3)}
              />
              <InputField
                label="Embed Batch Size"
                type="number"
                value={get("crawl", "embed_batch_size", 200)}
                onChange={(v) => update("crawl", "embed_batch_size", parseInt(v) || 200)}
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputField
                label="Scheduler Interval (sec)"
                type="number"
                value={get("crawl", "scheduler_interval_seconds", 300)}
                onChange={(v) => update("crawl", "scheduler_interval_seconds", parseInt(v) || 300)}
              />
              <InputField
                label="Max Retries"
                type="number"
                value={get("crawl", "max_retries", 2)}
                onChange={(v) => update("crawl", "max_retries", parseInt(v) || 2)}
              />
              <InputField
                label="Stale Timeout (min)"
                type="number"
                value={get("crawl", "stale_timeout_minutes", 30)}
                onChange={(v) => update("crawl", "stale_timeout_minutes", parseInt(v) || 30)}
              />
            </div>
            <InputField
              label="Max Continuous Rounds"
              type="number"
              value={get("crawl", "max_continuous_rounds", 10)}
              onChange={(v) => update("crawl", "max_continuous_rounds", parseInt(v) || 10)}
        ...
        <div
          ref={(el) => {
            sectionRefs.current["rate_limit"] = el;
          }}
          className="scroll-mt-6"
        >
          <SectionCard
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <InputField
                ...
                onChange={(v) => update("rate_limit", "default", v)}
